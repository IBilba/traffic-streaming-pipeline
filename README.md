...
Six containers run by default (`redpanda`, `topic-init` one-shot, `spark-master`, `spark-worker`, `spark-job`, `mongo`) plus the `uxsim-producer`. Two optional services land behind profile flags: `traffic-api` under `--profile api` and `dashboard` under `--profile dashboard`.
...
| `DASHBOARD_HOST` | `0.0.0.0` | Bind interface for the Streamlit dashboard. |
| `DASHBOARD_PORT` | `8501` | Dashboard port (mapped to host). |
...
### Enable the Streamlit dashboard (bonus)

```powershell
docker compose --profile dashboard up -d dashboard
```

Then open <http://localhost:8501>. Three tabs:

* **Overview**: raw-event count, current simulation time, top-N congested links, mean speed per link.
* **Network map**: live Plotly view of the 4x4 grid; line color = `vspeed` (red to green), line width = `vcount`. A slider scrubs through historical `t`.
* **Link detail**: per-link summary plus time series (per-step or 30s windowed).

The dashboard reads MongoDB directly through the same repository the REST API uses, so the `api` profile is not required.
...
```
traffic-streaming-pipeline/
├── docker-compose.yml          # Stack definition (6 services + api / dashboard profiles)
├── Dockerfile.producer         # Image for UXSIM producer
├── Dockerfile.api              # Image for FastAPI service
├── Dockerfile.dashboard        # Image for Streamlit dashboard
...
│   └── dashboard/              # Streamlit dashboard with Plotly network map (bonus)
